...
// DeploymentStatusManager is a simplified status manager for deployments without application records.
// Used for scenarios like appstore deployments where there's no application in the applications table,
// but we still want to track deployment status and send real-time notifications.
type DeploymentStatusManager struct {
	HandlerName      string
	DeploymentsStore *store_dynamodb.DeploymentsStore
	Pusher           *pusher.Client
	ApplicationId    string // Used as identifier (e.g., source URL for appstore)
	DeploymentId     string
}

func NewDeploymentStatusManager(handlerName string, deploymentsStore *store_dynamodb.DeploymentsStore, applicationId string, deploymentId string) *DeploymentStatusManager {
...
func (m *DeploymentStatusManager) WithPusher(pusher *pusher.Client) *DeploymentStatusManager {
	m.Pusher = pusher
	return m
}

func (m *DeploymentStatusManager) NewDeployment(ctx context.Context, deployment store_dynamodb.Deployment) error {
	if m.DeploymentsStore == nil {
		return fmt.Errorf("cannot create Deployment record: no DeploymentStore configured")
	}
	if err := m.DeploymentsStore.Insert(ctx, deployment); err != nil {
		return err
	}
	m.sendDeploymentStatusEvent(deployment.LastStatus, false)
	return nil
}

func (m *DeploymentStatusManager) SetErrorStatus(ctx context.Context, err error) string {
	if m.DeploymentsStore != nil {
		if deployStoreErr := m.DeploymentsStore.SetErrored(ctx, m.ApplicationId, m.DeploymentId); deployStoreErr != nil {
			log.Printf("warning: error setting errored on deployments table: %s\n", deployStoreErr.Error())
		}
	...
	msg := fmt.Sprintf("error: %s", err.Error())
	m.sendDeploymentStatusEvent(msg, true)
	return handlerError(m.HandlerName, err)
}

func (m *DeploymentStatusManager) sendDeploymentStatusEvent(status string, isErrorStatus bool) {
	if m.Pusher == nil {
		log.Printf("warning: no Pusher client configured")
		return
	}
	...
	if err := m.Pusher.Trigger(channel, events.ApplicationStatusEventName, event); err != nil {
		log.Printf("warning: error updating pusher channel %s with deployment status: %s: %s\n", channel, status, err.Error())
	}
}
